...
const MyProfileUser = ({ profile }) => {
  ...
  let Abiertas =
    Object.entries(AllHiring).length !== 0
      ? AllHiring.filter((h) => h.status === false)
      : [];
  console.log("abiertas", Abiertas);
  let Cerradas =
    Object.entries(AllHiring).length !== 0
      ? AllHiring.filter((h) => h.status === true)
      : [];
  ...
  return (
    <Container>
      ...
      <ContainerContratos>
        <NavBar>
          <BtnPublic onClick={() => setPanelHiring("activas")}>
            Activas
          </BtnPublic>
          <BtnPublic onClick={() => setPanelHiring("historial")}>
            Historial{" "}
          </BtnPublic>
        </NavBar>
        <NavHiring>
